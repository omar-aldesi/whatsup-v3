    ...
    <div class="container-fluid p-0">
		<div class="card">
			<div class="card-header">
				<h4 class="card-title">{{__($channel)}} {{ translate('Campaigns')}}</h4>
			</div>
				...
				<form action="{{route('admin.campaign.search')}}" method="post">
					...
					<div class="filter-form">
						...
						<div class="filter-action">
							<button class="i-btn primary--btn btn--md" type="submit"><i class="fas fa-search"></i> {{ translate('Search')}}</button>
							@php
								$routeName =  "admin.campaign.".strtolower($channel);
							@endphp
							<button class="i-btn danger--btn btn--md" type="submit">
								<a  class="i-btn danger--btn btn--md" href="{{route($routeName)}}" class="text-white"><i class="fas fa-sync"></i> {{ translate('Reset')}}</a>
							</button>
							<a href="{{route('admin.campaign.create',strtolower($channel))}}" class="i-btn info--btn btn--md">
						...
						@forelse($campaigns as $campaign)
							<tr class="@if($loop->even)@endif">
								...
								<td data-label="{{ translate('Contacts')}}">
									<a href="{{route('admin.campaign.contacts',$campaign->id)}}" class="badge badge--primary p-2"> {{ translate('view Contact')}} ({{$campaign->contacts->count()}}) </a>
								</td>
								...
								<td data-label="{{ translate('Status')}}">
									...
									@elseif($campaign->status == 'DeActive')
										<span class="badge badge--danger">{{ translate('DeActive')}}</span>
								...
								<td data-label={{ translate('Action')}}>
									<div class="d-flex align-items-center justify-content-md-start justify-content-end gap-3">
										<a href="{{route('admin.campaign.edit',['type' => strtolower($channel), 'id' => $campaign->id])}}" class="i-btn info--btn btn--sm"><i class="las la-pen"></i>
										</a>
