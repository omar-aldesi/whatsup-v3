		...
		<div class="card-header">
			...
			<div>
				<div class="d-flex align-items-center justify-content-start justify-content-md-end flex-wrap gap-3">
					...
					<div>
						@if(@$group)
							<a href="{{route('admin.contact.email.own.group.export', $group->id)}}" class="w-100 d-block text-center i-btn info--btn btn--md border-0 px-1 py-2 rounded ms-2"><i class="las la-cloud-download-alt"></i> {{ translate('Export Contact')}}</a>
						@else
							<a href="{{route('admin.contact.email.own.export')}}" class="w-100 d-block ext-center i-btn info--btn btn--md border-0 px-1 py-2 rounded ms-2"><i class="las la-cloud-download-alt"></i> {{ translate('Export Contact')}}</a>
						@endif
					</div>
				</div>
			</div>
		...
		<div class="card-filter">
			<form action="{{route(request()->route()->getName(),@$group->id)}}" method="get">
				...
				<div class="filter-form">
					...
					<div class="filter-item">
						<input type="text" autocomplete="off" name="search" placeholder="{{translate('Search with Email or Name')}}" class="form-control" id="search" value="{{@$search}}">
					</div>
