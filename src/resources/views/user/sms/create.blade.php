                    ...
                    <div class="note-container col-xl-3 order-xl-2 order-1 d-xl-block d-none">
                        ...
                        @if(auth()->user()->sms_gateway == 1)
                            <div class="form-wrapper select-sms-gateway mt-3">
                                <h6 class="form-wrapper-title" title="{{ translate('If left unselected then the default gateway will be selected') }}">{{ $allowed_access->type == App\Models\PricingPlan::USER ? translate("Select User's SMS Gateway") : translate("Select Admin's SMS Gateway")}}</h6>
