...
<section>
	<div class="card">
		<div class="card-header">
			<h4 class="card-title">{{ translate('Select Email Contact Options')}}</h4>
			<div class="d-flex align-items-center justify-content-md-end justify-content-start flex-wrap gap-3">
				...
				<div>
					@if(@$group)
						<a href="{{route('user.email.contact.group.export', $group->id)}}" class="i-btn info--btn btn--md"><i class="las la-cloud-download-alt"></i> {{ translate('Export Contact')}}</a>
					@else
						<a href="{{route('user.email.contact.export')}}" class="i-btn info--btn btn--md"><i class="las la-cloud-download-alt"></i> {{ translate('Export Contact')}}</a>
					@endif
				</div>
